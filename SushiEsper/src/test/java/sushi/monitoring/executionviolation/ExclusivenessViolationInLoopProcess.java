 ...
 * @author micha
 */
public class ExclusivenessViolationInLoopProcess extends AbstractMonitoringTest{
	...
	protected void simulate(List<SushiEventType> eventTypes) {
		...
		for(SushiEventType eventType : eventTypes){
			...
			//Alle Eventtypen werden gesendet, also werden beide XOR-Pfade simuliert --> ExclusivenessViolation
	...
	protected void assertQueryStatus() {
		...
		for(SushiProcessInstance processInstance : SushiProcessInstance.findAll()){
			...
			assertTrue(exclusivenessViolationStatusContained);
		}
	}
