...
public class SushiEsperTests {
	...
	public void setup(){
		...
		assertTrue(SushiEsper.instanceIsCleared());
	...
	public void testSushiQuery(){
		...
		SushiEvent.setEventType(events, eventType);
		Broker.send(events);
		String log = query.execute(esper);
	...
	public void testWindowCreation(){
		assertTrue("Window has already been created", ! SushiEsper.getInstance().hasWindow(eventType.getTypeName() + "Window"));
		SushiEsper.getInstance().createWindow(eventType);
		assertTrue("Window has not been created", SushiEsper.getInstance().hasWindow(eventType.getTypeName() + "Window"));
		//Send Events
		...
		EPOnDemandQueryResult result = SushiEsper.getInstance().getEsperRuntime().executeQuery("Select * From KinoWindow");
