...
import sushi.xml.importer.XSDNameSpaceContext;
...
public class EdifactParser {
   ...
   * @throws IOException
   */
  private static EdifactParser instance = null;
  
  public static EdifactParser getInstance() {
    if (instance == null) {
      instance = new EdifactParser();
    }
    return instance;
  }
...
//    System.out.println(message);
    ...
    if (message.contains("COPINO:D:95B:UN:INT10I")) {
      //This file is momentarily located at my Dropbox
      //since the path cannot be resolved by smooks if java is started from a different Project than SushiImport
      //TODO: Move file to server (but then it of course would only be accessible within the HPI or find other solution
...
//      Smooks smooks_copino = new Smooks("smooks-config-copino.xml");
  ...
  private static String getFilePathForEdifactXSD(String id, String version,String release) {
    String path = EdifactParser.class.getResource("").toString();
