...
public class CorrelatorTest {
	...
	public void testCorrelator(){
		...
		Correlator.correlate(correlationEventTypes, Arrays.asList(correlationAttribute), process, null);
	...
	public void testCorrelatorWithOutEvents(){
		...
		//Korrelieren
		SushiAttribute correlationAttribute = new SushiAttribute("Location", SushiAttributeTypeEnum.INTEGER);
		Correlator.correlate(correlationEventTypes, Arrays.asList(correlationAttribute), processFromDataBase, null);
	...
	private void assertNumberOfDataSets(int events, int eventTypes, int processes, int processInstances){
		assertTrue(SushiEvent.findAll().size() == events);
		assertTrue(SushiEventType.findAll().size() == eventTypes);
		assertTrue(SushiProcess.findAll().size() == processes);
